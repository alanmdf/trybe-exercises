...
SELECT 
    JOB_ID, ((MIN_SALARY + MAX_SALARY) / 2) AS `Média salarial`
FROM
    jobs
...
SELECT 
    SUM(SALARY) AS `Custo da folha de pagamento`
FROM
    employees;
...
SELECT 
    COUNT(JOB_ID) AS `Qtd de Pessoas Programadoras`
FROM
    employees
WHERE
    JOB_ID = 'IT_PROG';
...
SELECT 
    JOB_ID, SUM(SALARY) as `Folha de pagamento por profissão`
FROM
    employees
GROUP BY JOB_ID;
...
SELECT 
    JOB_ID, AVG(SALARY) AS `Média salarial por cargo`
FROM
    employees
GROUP BY JOB_ID
HAVING JOB_ID <> 'IT_PROG'
ORDER BY `Média salarial por cargo` DESC;

-- 10. Escreva um query que exiba média salarial e o número de funcionários de todos os departamentos com mais
-- de dez funcionários. Dica: agrupe pelo department_id .

SELECT 
    DEPARTMENT_ID,
    AVG(SALARY) AS `Média Salarial por Departamento`,
    COUNT(*) AS `Qtd de funcionarios`
FROM
    employees
GROUP BY DEPARTMENT_ID
HAVING `Qtd de funcionarios` > 10;
...
WHERE IF (PHONE_NUMBER LIKE '515%', REPLACE(PHONE_NUMBER, '515', '777'), PHONE_NUMBER);
...
SELECT 
    EMPLOYEE_ID, FIRST_NAME, YEAR(HIRE_DATE) `hiring_year`
FROM
    employees;
...
SELECT 
    EMPLOYEE_ID, FIRST_NAME, DAY(HIRE_DATE) `hiring_day`
FROM
    employees;
...
SELECT 
    EMPLOYEE_ID, FIRST_NAME, MONTH(HIRE_DATE) `hiring_month`
FROM
    employees;
